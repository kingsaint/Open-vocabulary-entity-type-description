...
OUTPUT_DIM =  preprocess.vocab_size
EMBEDDING_DIM = 100
HIDDEN_DIM = 100
...
class PositionalFactEncoder(nn.Module):
    ...
    def forward(self, embedded_sentence, fact_lengths):
        ...
        return torch.sum(weighted, dim=1).squeeze(1) # sum with tokens
...
class OutputModule(nn.Module):
    def __init__(self):
        ...
        self.attn = nn.Linear(EMBEDDING_DIM + HIDDEN_DIM, MAX_CONTEXT_LEN)
        self.attn_combine = nn.Linear(EMBEDDING_DIM + HIDDEN_DIM, HIDDEN_DIM)
        self.hidden2output = nn.Linear(HIDDEN_DIM, OUTPUT_DIM)

    def forward(self, decoder_input, hidden, fact_embeddings):
        attn_weights = F.softmax(self.attn(torch.cat((decoder_input, hidden.squeeze(1)), dim=1)), dim=1)
        attn_applied = torch.mm(attn_weights,fact_embeddings)
        input2attn_combine = torch.cat((decoder_input, attn_applied), 1)
        input2gru = self.attn_combine(input2attn_combine).unsqueeze(0)
        input2gru = F.relu(input2gru)
        gru_out, hidden = self.gru(input2gru, hidden)
        gru_out = gru_out.view(1,-1)
        output = self.hidden2output(gru_out)
        output = F.log_softmax(output, dim=1)
        return output, hidden

class Seq2seq_w_Attention(nn.Module):  # Description Generation Network
    def __init__(self):
        super(Seq2seq_w_Attention, self).__init__()
        self.word_embeddings = nn.Embedding(num_embeddings = INPUT_DIM,embedding_dim = EMBEDDING_DIM, padding_idx=0)
...
flag = 'test'
print("Testing...")
fout = open('result_Fact2seq_w_attention.csv', 'w+')
